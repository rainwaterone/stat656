...
attribute_map = {
        'nhtsa_id':[DT.ID, ('')], 
        'Year':    [DT.Nominal, (2003, 2004, 2005, 2006, 2007, 2008, 
                                 2009, 2010, 2011)],
        'make':    [DT.Nominal, ('CHEVROLET', 'PONTIAC', 'SATURN')],
        'model':   [DT.Nominal, ('COBALT', 'G5', 'HHR', 'ION', 'SKY', 
                                 'SOLSTICE')],
        'description': [DT.Text,('')],
        'crashed': [DT.Binary,  ('N', 'Y')],
        'abs':     [DT.Binary,  ('N', 'Y')],
...
n_int = 11        # interval variable - points, excluding the target
n_nom = [9, 3, 6] # two nominal variables (Region, and topic)
...
#**************** STEPWISE SELECTION *****************************************  
print("\n{:-<40s}{:->39s}".format('*', '*'))
print("{:-<25s}     STEPWISE REGRESSION     {:->25s}\n".format('*', '*'))
...
print("\n{:-<25s} STEPWISE 70/30 VALIDATION {:->27s}".format('*','*'))
...
#**************** ENSEMBLE           ***************************************** 
ecl = VotingClassifier(estimators=[('Random Forest', rfc), 
                                   ('Decision Tree', dtc),   
                                   ('Log. Reg', lrg)],   
                                   voting='soft' )
with warnings.catch_warnings():
